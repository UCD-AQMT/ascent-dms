...
downloadServer <- function(id, site) {
  moduleServer(id, function(input, output, session) {
    ...
    xact_l1a <- reactive({
      ...
      results <- ascentr::xact_l1a_df(site(), input$dates[1], input$dates[2], con)
      
      if (input$metadata) {
        meta <- ascentr::xact_l1a_metadata(site(), input$dates[1], input$dates[2], con,
    ...
    smps_l0 <- reactive({
      ...
      datasets <- ascentr::smps_datasets(site(), input$dates[1], input$dates[2], con)
      params <- datasets |>
      ...
      aim_file_data <- function(ds_value, start_time, end_time, site, cols, con, n) {
        ...
        meta <- ascentr::smps_metadata(site, start_time, end_time, con)
        df <- ascentr::smps_data(site, start_time, end_time, con)
        # build aim file
        shiny::incProgress(1/n, message = paste("Constructing", n, 
                                                "AIM files for download.", 
                                                "Thank you for your patience."))
        file <- ascentr::build_aim_file(meta, df, ds_value, cols)
        
      }
      cols <- ascentr::smps_columns(con)
      withProgress(message = "Constructing AIM files", value = 0, {
        n <- nrow(params)
        dat <- purrr::pmap(params, aim_file_data, site = site(), cols = cols, con = con, n = n)
    ...
    smps_l1a <- reactive({
      df <- ascentr::smps_l1a_df(site(), input$dates[1], input$dates[2], con)
      if (input$metadata) {
        meta <- ascentr::smps_l1a_metadata(site(), input$dates[1], input$dates[2], con)  
        temp_dir <- file.path(tempdir(), as.integer(Sys.time()))
        dir.create(temp_dir)
        txt_file <- file.path(temp_dir, paste0(filename_noext(), ".txt"))
        writeLines(meta, txt_file)
    ...
    ae33_l1a <- reactive({
      ...
      results <- ascentr::ae33_l1a(site(), input$dates[1], input$dates[2], ae33_con)
      
      if (input$metadata) {
        
        metadata <- ascentr::ae33_l1a_metadata(site(), input$dates[1], input$dates[2], con)
        temp_dir <- file.path(tempdir(), as.integer(Sys.time()))
        dir.create(temp_dir)
        txt_file <- file.path(temp_dir, paste0(filename_noext(), ".txt"))
    ...
    output$download <- downloadHandler(
      filename = function() {
        ...
        base_filename <- glue::glue("ASCENT_{site()}_{input$instrument}_",
    ...
    output$meta_text <- renderPrint({
      ...
      m <- ascentr::basic_metadata(site(), input$instrument, input$dates[1], input$dates[2],
                                   input$level, con)
      m
    })
